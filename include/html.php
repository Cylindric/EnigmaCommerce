...
function start_page(&$objpage, $page, $pagetype=ENIGMA_HTML) {
    ...
    switch ($pagetype) {
        ...
        case ENIGMA_TEXT:
            $objpage = new XTemplate(dirname(dirname(__FILE__)) . "/text/$page.html");
            $objpage->assign('PAGE_TITLE', $OPT->pagetitle);
            break;
        ...
        case ENIGMA_ARTICLE:
            $objpage = new XTemplate(dirname(dirname(__FILE__)) . "/articles/$page.html");
            $objpage->assign('PAGE_TITLE', $OPT->pagetitle);
            break;
    }
...
function mosModuleText($moduleid = 0, &$title) {
    // Skip any Mambo/Joomla references...
    return "mos_moduletext for module $moduleid, $title";
...
function mostext($categoryid = 0) {
    ...
    // Skip any Mambo/Joomla references...
    return "mos_text for category $categoryid";
...
function draw_select($selectname, $table, $keyfield, $valuefield, $selectedkey=0, $zerotext='', $tabid=-1) {
    ...
    if (strlen($zerotext) > 0)
        $returnval .= "<option value=\"$selectedkey\" selected=\"selected\">$zerotext</option>";
    $rs = db_query($sql);
    while ($r = db_fetch_array($rs)) {
        $returnval .= "<option value=\"" . $r[$keyfield] . "\"";
        if ($r[$keyfield] == $selectedkey)
...
function draw_selectcategory($selectname, $selectedkey=-1, $zerotext='', $parentid=-1) {
    ...
    if ($selectedkey == -1)
        $selected = ' selected="selected"';
    if (strlen($zerotext) > 0)
        $returnval .= "<option value=\"$selectedkey\"$selected>$zerotext</option>";
...
function draw_rateselect($selectname, $startval, $endval, $step, $currentval, $baseval) {
    ...
    for ($i = $startval; $i <= $endval; $i+=$step) {
        ...
        $selected = '';
        if (($currentadded == false) && ($i == $currentbal))
            $selected = ' selected="selected"';
