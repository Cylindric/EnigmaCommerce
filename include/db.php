...
//==============================================================================
// DIRECT DATABASE INTERACTIONS.  CHANGE THESE IF YOU ARE USING A DIFFERENT DB
//==============================================================================
	...
	* $dbuser and $dbpass. */
		...
		if (! $dbh = mysql_pconnect($dbhost, $dbuser, $dbpass)) {
			if ($DB_DEBUG) {
				echo "<h2>1.Can't connect to $dbhost as $dbuser</h2>";
		...
		if (! mysql_select_db($dbname)) {
			if ($DB_DEBUG) {
				echo "<h2>2.Can't select database $dbname</h2>";
				echo "<p><b>MySQL Error</b>: ", mysql_error();
			} else {
				echo "<h2>Database error encountered</h2>";
			}
	...
	* occurred */
		...
		global $DB_DIE_ON_FAIL, $DB_DEBUG, $DB_KEEP_SQL, $DB_LAST_SQL, $DB_ALL_PREVIOUS_SQL;

		if ($DB_KEEP_SQL) {
			$DB_LAST_SQL = $query;
			$DB_ALL_PREVIOUS_SQL .= htmlspecialchars($query)."\n";
		}
		...
		if (! $qid && ! $silent) {
			if ($DB_DEBUG) {
				echo "<h2>3.Can't execute query</h2>";
				echo "<pre>" . htmlspecialchars($query) . "</pre>";
				echo "<p><b>MySQL Error</b>: ", mysql_error();
			} else {
				echo "<h2>Database error encountered</h2>";
	...
	* $qid */
	...
	function db_escape_string($stringval) {
		return mysql_real_escape_string($stringval);
	}
