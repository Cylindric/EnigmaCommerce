...
class SluggableBehavior extends ModelBehavior {
    ...
    public function beforeSave($model) {
        ...
        if ((!$settings['real'] || $modelHasSlugField) && ($settings['overwrite'] || $recordIsNew)) {
            ...
            if (!empty($label)) {
                $slug = $this->slug($label, $settings);
                $conditions = array($model->alias . '.' . $settings['slug'] . ' LIKE' => $slug . '%');
                if (!empty($model->id)) {
                    $conditions['not'] = array($model->alias . '.' . $model->primaryKey => $model->id);
                }

                if (!empty($settings['unique_conditions'])) {
                    foreach ($settings['unique_conditions'] as $cond) {
                        if (isset($model->data[$model->alias][$cond['localField']])) {
                            $conditions[$cond['foreignField']] = $model->data[$model->alias][$cond['localField']];
                        }
                    }
                }
     ...
     */
    protected function slug($string, $settings) {
        ...
        if (!empty($settings['translation']) && is_array($settings['translation'])) {
            ...
            return $this->slug(
                    $string, array_merge($settings, array('translation' => $this->translations[$settings['translation']]))
            );
        }
        ...
        $string = preg_replace(
                '/' . preg_quote($settings['separator']) . '[' . preg_quote($settings['separator']) . ']*/', $settings['separator'], $string
        );
    ...
    protected function initialiseTranslations() {
