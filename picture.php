<?php
	...
	if(!empty($_POST)) {
		...
		$newfilepath = $_SERVER['DOCUMENT_ROOT'] . $OPT->productimageroot;
		$newfilename = str_pad($newpictureid, $OPT->pictureidlength, '0', STR_PAD_LEFT) . $fileextention;
		$newfile = $newfilepath.$newfilename;

		if(move_uploaded_file($tempfile, $newfile)) {
			echo ("Added picture $newfile<br>");
			//reload_page();
