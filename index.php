...
// Category
			case 'category':
				...
				//log activity later
				if($_SESSION['maincatid']!=$PAGEOPT['maincatid']) $addlog['maincat'] = true;
				if($_SESSION['subcatid']!=$PAGEOPT['subcatid'])   $addlog['subcat'] = true;
...
// Item
			case 'item':
				...
				//log activity later
	 			if($_SESSION['itemid']!=$PAGEOPT['itemid']) $addlog['item'] = true;
...
// ----------------------------------------------------------------------------
	...
	//generic html header
	start_page($html,'head'); end_page($html);
	...
	//otherwise a "normal" page
		...
		*/
 		...
 		if(ParentCategoryItemCount(144)>0) $html->parse('main.bulkbuys');
 		if(ParentCategoryItemCount(151)>0) $html->parse('main.keyproducts');

		if($PAGEVIS['MiniSearch'])	$html->parse('main.minisearch');
		if($PAGEVIS['Brands'])		$html->parse('main.brands');
		if($PAGEVIS['SecureLogo'])	$html->parse('main.securelogo');

		$html->assign('PAGE_HEADING', 'Koi Logic');
		$html->assign('FOCUS_FORM', '**~FOCUS_FORM~**');
		$html->assign('FOCUS_FIELD', '**~FOCUS_FIELD~**');
		if($PAGEVIS['SubCategories']) $html->assign('PAGE_HEADING', 'Please click a sub-category:');
		if($PAGEVIS['ItemList'])      $html->assign('PAGE_HEADING', '&nbsp;');
		if($PAGEVIS['Search'])        $html->assign('PAGE_HEADING', 'Search Results');
		if($PAGEVIS['FullBasket'])    $html->assign('PAGE_HEADING', 'Checkout');
		if($PAGEVIS['Item'])          $html->assign('PAGE_HEADING', '**~PAGE_HEADING~**');
		if($PAGEVIS['Article'])       $html->assign('PAGE_HEADING', '**~PAGE_HEADING~**');
		if($PAGEVIS['Checkout'])      $html->assign('PAGE_HEADING', 'Checking out...');
		if($PAGEVIS['WebLog'])        $html->assign('PAGE_HEADING', 'Web Log');
		if($PAGEVIS['EditPictures'])  $html->assign('PAGE_HEADING', 'Pictures');
		if($PAGEVIS['EditList'])      $html->assign('PAGE_HEADING', 'Edit List');
		if($PAGEVIS['StockTake'])     $html->assign('PAGE_HEADING', 'Stock Taking');
		...
		if($PAGEVIS['MiniBasket'])		require('basket.php');
		if($PAGEVIS['KLFilterBox'])		$html->parse('main.klfilter');
		...
		if($PAGEVIS['RightPanels'])		require('rightpanels.php');
		...
		if($PAGEVIS['Legend']) $html->parse('main.legend');
		end_page($html);
